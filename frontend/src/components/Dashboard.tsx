...
import { TrendingUp, TrendingDown, DollarSign, Activity, Users, BarChart3, Zap, Shield, RefreshCw } from 'lucide-react';
...
const Dashboard: React.FC<DashboardProps> = ({ user, token }) => {
  ...
  useEffect(() => {
    ...
    if (marketData.length > 0) {
      ...
      if (btcData) {
        generateTradingChart(parseFloat(btcData.price.replace(/,/g, '')));
      }
    }
  ...
  const fetchSystemStats = async () => {
    try {
      const response = await fetch('http://localhost:3001/api/monitoring/stats');
      if (response.ok) {
        const data = await response.json();
        setSystemStats(data.data);
        
        // Gerar dados de performance baseados nas estatísticas reais
        generatePerformanceChart(data.data);
      }
    } catch (err) {
  ...
  const generatePerformanceChart = (stats: SystemStats) => {
  ...
  const fetchMarketData = async () => {
    try {
      ...
      if (response.ok) {
        ...
        const formattedData: MarketData[] = data.data.map((item: any) => ({
          symbol: item.symbol,
        ...
        // Criar dados de gráfico baseados no BTC
        const btcData = data.data.find((item: any) => item.symbol === 'BTCUSDT');
        if (btcData) {
  ...
  const generateTradingChart = (currentPrice: number) => {
    // Gerar dados de gráfico baseados no preço atual (simulando variações históricas)
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-blue-900 to-slate-900 p-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-blue-200">
              Bem-vindo, {user?.name} • {new Date().toLocaleDateString('pt-BR')}
            </p>
          </div>
              ...
              <tbody>
                {marketData.map((item, index) => (
                  <tr key={index} className="border-b border-white/10 hover:bg-white/5">
                    ...
                    <td className="py-3 px-4 text-right text-blue-200">{item.volume}</td>
      ...
      <div className="mt-8 text-center text-blue-200 text-sm">
        <p>AURA Trading System v1.0.0 • Desenvolvido com ❤️ • Status: Operacional</p>
      </div>
    </div>
  );
};
